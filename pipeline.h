...
public:
  pipeline_t(sim_t *_sim, mmu_t *_mmu, uint32_t _id, uint32_t fq_size,
             uint32_t num_chkpts, uint32_t rob_size, uint32_t prf_size,
             uint32_t iq_size, uint32_t iq_num_parts, uint32_t lq_size,
             uint32_t sq_size, uint32_t fetch_width, uint32_t dispatch_width,
             uint32_t issue_width, uint32_t retire_width,
             uint32_t fu_lane_matrix[], uint32_t fu_lat[]);
  ...
  bool step_micro(size_t instret_limit,
                  size_t &instret); // Step the pipeline 1 cycle.
  ...
  virtual void disasm(insn_t insn,
                      reg_t pc); // disassemble and print an instruction
  void disasm(insn_t insn, reg_t pc,
              FILE *out = stderr); // disassemble and print an instruction
  void disasm(insn_t insn, cycle_t cycle, reg_t pc, uint64_t seq,
              FILE *out = stderr); // disassemble and print an instruction
...
private:
  ...
  unsigned int
      fu_lane_matrix[(unsigned int)NUMBER_FU_TYPES]; // Indexed by FU type: bit
                                                     // vector indicating which
                                                     // lanes have that FU type.
  unsigned int fu_lane_ptr[(
      unsigned int)NUMBER_FU_TYPES]; // Indexed by FU type: lane to which the
                                     // last instruction of that FU type was
                                     // steered.
  ...
  void check_double(reg_t micro0, reg_t micro1, reg_t isa0, reg_t isa1,
                    const char *desc);
