...
// constructor
issue_queue::issue_queue(unsigned int size, unsigned int num_parts,
                         pipeline_t *_proc)
    : proc(_proc) {
  ...
  // Issue queue is initially empty, so oldest and youngest indices are
  // initially invalid.
  oldest = -1;
  youngest = -1;
...
bool issue_queue::stall(unsigned int bundle_inst) {
  ...
  ifprintf(logging_on, proc->dispatch_log,
           "ISSUE_QUEUE: Checking for stall fl_length: %u  length: %u "
           "bundle_inst: %u stall: %s\n",
           fl_length, length, bundle_inst,
           (fl_length < bundle_inst) ? "true" : "false");
  return (fl_length < bundle_inst);
}

void issue_queue::dispatch(unsigned int index, unsigned long long branch_mask,
                           unsigned int lane_id, bool A_valid, bool A_ready,
                           unsigned int A_tag, bool B_valid, bool B_ready,
                           unsigned int B_tag, bool D_valid, bool D_ready,
                           unsigned int D_tag) {
  unsigned int free;
  ...
  for (unsigned int i = 0; i < size; i++) {
    if (q[i].valid) { // Only consider valid issue queue entries.
      if (q[i].A_valid && (tag == q[i].A_tag)) { // Check first source operand.
        ...
        LOG(proc->issue_log, proc->cycle, proc->PAY.buf[q[i].index].sequence,
            proc->PAY.buf[q[i].index].pc, "Waking up RS1 iq entry %u", i);
...
#ifdef RISCV_MICRO_DEBUG
        LOG(proc->issue_log, proc->cycle, proc->PAY.buf[q[i].index].sequence,
            proc->PAY.buf[q[i].index].pc, "Waking up RS2 iq entry %u", i);
        dump_iq(proc, i, proc->issue_log);
#endif
      }
...
#ifdef RISCV_MICRO_DEBUG
        LOG(proc->issue_log, proc->cycle, proc->PAY.buf[q[i].index].sequence,
            proc->PAY.buf[q[i].index].pc, "Waking up RS3 iq entry %u", i);
        dump_iq(proc, i, proc->issue_log);
#endif
      }
    }
...
void issue_queue::select_and_issue(unsigned int num_lanes,
  ...
  if (IDEAL_AGE_BASED) {
    if (oldest == -1) { // IQ empty, so no age-based list to sequence through.
      ...
      i = (unsigned int)
          oldest; // Start sequencing at oldest instruction in the IQ.
    }
  } else {
    i = part_next;
  }
  ...
  for (j = 0; j < size; j++) {
    ...
    if (q[i].valid && (!q[i].A_valid || q[i].A_ready) &&
        (!q[i].B_valid || q[i].B_ready) && (!q[i].D_valid || q[i].D_ready)) {
      if (PRESTEER) {
        ...
        while (!issue && (dyn_lane_id < num_lanes)) {
          if ((q[i].lane_id & (1 << dyn_lane_id)) &&
              !Execution_Lanes[dyn_lane_id].rr.valid) {
            issue = true;
      ...
      if (issue) {
        ...
        // Issue the instruction to the Register Read Stage within the Execution
        // Lane.
    ...
    if (IDEAL_AGE_BASED) {
      // Set q index to that of the next-oldest instruction, or break from loop
      // if there is no next-oldest instruction. Note: even if we issued and
      // removed i from the IQ, above, its next pointer is still available.
      if (q[i].next == -1)
        break;
      else
...
void issue_queue::dump_iq(pipeline_t *proc, unsigned int index, FILE *file) {
  proc->disasm(proc->PAY.buf[q[index].index].inst, proc->cycle,
               proc->PAY.buf[q[index].index].pc,
               proc->PAY.buf[q[index].index].sequence, file);
  ifprintf(logging_on, file, "fl_head %d fl_tail %d fl_length %d\n", fl_head,
           fl_tail, fl_length);
  ifprintf(logging_on, file, "valid      : %u\t", q[index].valid);
  ifprintf(logging_on, file, "branch_mask: %" PRIu64 "\t",
           q[index].branch_mask);
