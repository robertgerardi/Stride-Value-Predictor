...
typedef enum {
  ...
  SEL_IQ_NONE, // Skip IQ, mark completed right away. Set this if detected any
               // exceptions before the DISPATCH stage.
} sel_iq;
...
typedef struct {
  ...
  reg_t next_pc; // The next instruction's PC. (I.e., the PC of the instruction
                 // fetched after this one.)
  bool branch;   // This instruction was identified as a branch, by the BTB (if
                 // bundle came from instr. cache) or by the trace cache.
  btb_branch_type_e branch_type; // If the instruction was identified as a
                                 // branch, this is its type.
  uint64_t
      branch_target; // If the instruction was identified as a branch, this is
                     // its taken target (not valid for indirect branches).
  ...
  unsigned int
      LQ_index; // Indices into LSU. Only used by loads, stores, and branches.
  bool LQ_phase;
  ...
  // ADDED VALYES PREDICTION VARIABLES
  union64_t C_predvalue;
  bool C_prediction;
  uint64_t VPQIndex;
  bool VPQPhase;
  bool VPQValidEntry;

  bool ValPredEligible;
  bool IneligibleType; // 0 is type, 1 is drop
  bool ValPredConfidence;
  bool ValPredCorrectness;
  bool SVPHit;
  bool branchSqaush;

  uint32_t fflags; // If it is a FP instruction, this is the new fflags bits it
                   // will post
...
public:
  ...
  // Perfect branch prediction, up to max_length instructions or the first
  // indirect branch.
  void predict(pipeline_t *proc, uint64_t pc, uint64_t max_length,
               uint64_t &cb_predictions, uint64_t &indirect_target);
