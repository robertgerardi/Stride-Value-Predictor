...
void pipeline_t::retire(size_t &instret) {
  ...
  // FIX_ME #17a BEGIN
  head_valid =
      REN->precommit(completed, exception, load_viol, br_misp, val_misp, load,
...
/*
    if (PAY.buf[PAY.head].ValPredConfidence && PAY.buf[PAY.head].ValPredCorrectness){
      VPUnit.ConfCorrect += 1;
    }
    else if(PAY.buf[PAY.head].ValPredConfidence && !PAY.buf[PAY.head].ValPredCorrectness){
      VPUnit.ConfIncorrect += 1;
    }
    else if(!PAY.buf[PAY.head].ValPredConfidence && PAY.buf[PAY.head].ValPredCorrectness){
      VPUnit.UnconfCorrect += 1;
    }
    else if(!PAY.buf[PAY.head].ValPredConfidence && !PAY.buf[PAY.head].ValPredCorrectness){
      VPUnit.UnconfIncorrect += 1;
    }
*/
    ...
    if (VAL_PREDICTION){
      if (PAY.buf[PAY.head].ValPredEligible){
        VPUnit.EligiblePred += 1;
      }
      else{
        VPUnit.IneligiblePred += 1;
        if (PAY.buf[PAY.head].IneligibleType){
          VPUnit.IneligibleDrop += 1;
        }
        else{
          VPUnit.IneligibleType += 1;
        }
      }

      if (!PAY.buf[PAY.head].SVPHit && PAY.buf[PAY.head].ValPredEligible && (PAY.buf[PAY.head].VPQValidEntry || PERFECT_VALPRED)){
        VPUnit.VPMiss += 1;
      }

      if (PAY.buf[PAY.head].SVPHit){
        if ((PAY.buf[PAY.head].VPQValidEntry || PERFECT_VALPRED) && PAY.buf[PAY.head].ValPredEligible && PAY.buf[PAY.head].ValPredConfidence && PAY.buf[PAY.head].ValPredCorrectness){
          VPUnit.ConfCorrect += 1;
        }
        else if(PAY.buf[PAY.head].VPQValidEntry && PAY.buf[PAY.head].ValPredEligible && PAY.buf[PAY.head].ValPredConfidence && !PAY.buf[PAY.head].ValPredCorrectness){
          VPUnit.ConfIncorrect += 1;
        }
        else if(PAY.buf[PAY.head].VPQValidEntry && PAY.buf[PAY.head].ValPredEligible && !PAY.buf[PAY.head].ValPredConfidence && PAY.buf[PAY.head].ValPredCorrectness){
          VPUnit.UnconfCorrect += 1;
      ...
      // FIX_ME #17b BEGIN
      int tempPC = REN->getPC();
      REN->commit();

      if (PAY.buf[PAY.head].VPQValidEntry) {
        assert((IS_LOAD(PAY.buf[PAY.head].flags) ||
                IS_FPALU(PAY.buf[PAY.head].flags) ||
                IS_INTALU(PAY.buf[PAY.head].flags)));

        VPUnit.retire_val(tempPC, PAY.buf[PAY.head].VPQIndex);
      }
      ...
      if (load || store) {
        assert(load != store); // Make sure that the same instruction does not
                               // have both flags set.
        assert(
            !PAY.buf[PAY.head].split_store ||
            !PAY.buf[PAY.head].upper); // Assert that the second uop of a
                                       // split-store signals the LSU to commit.
      ...
      // Split instructions should only count as one architectural instruction,
      // therefore, only the second uop should increment the count.
      ...
      if (amo || csr) { // Resume the stalled fetch unit after committing a
        ...
        PAY.pop();
                                        ...
                                        // a value-mispredicted instruction, if
                                        // "approach #1 recovery" is configured.
        reg_t next_inst_pc;
        if (br_misp)
      ...
      // (not value prediction). Therefore the load is incorrect and not
      // committed.
      assert(load);
