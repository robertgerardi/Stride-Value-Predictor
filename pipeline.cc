  ...
  LSU(lq_size, sq_size, Tid, _mmu, this)
{
 ...
 if (!PERFECT_VALPRED){
      ...
      fprintf(stats_log, "  oracleconf      = %d\n", (ORACLE_CONF ? 1 : 0));
      fprintf(stats_log, "  # index bits    = %d\n", SVP_INDEX_BITS);
      fprintf(stats_log, "  # tag bits      = %d\n", SVP_TAG_BITS);
      fprintf(stats_log, "  confmax         = %d\n", CONF_MAX);
      fprintf(stats_log, "  confinc         = %d\n", CONF_INC);
      fprintf(stats_log, "  confdec         = %d\n", CONF_DEC);
      fprintf(stats_log, "  replace_stride  = %d\n", REPLACE_STRIDE);
      fprintf(stats_log, "  replace         = %d\n", REPLACE_CONF_THRESHOLD);
      fprintf(stats_log, "  predINTALU      = %d\n", (PRED_INTALU ? 1 : 0));
      fprintf(stats_log, "  predFPALU       = %d\n", (PRED_FPALU ? 1 : 0));
      fprintf(stats_log, "  predLOAD        = %d\n", (PRED_LOAD ? 1 : 0));
      fprintf(stats_log, "  VPQ_full_policy = %d\n", (VPQ_FULL_POLICY ? 1 : 0));

      fprintf(stats_log, "\nCOST ACCOUNTING\n");

      fprintf(stats_log, "  One SVP entry: \n");
      fprintf(stats_log, "    tag           : %d bits\n", SVP_TAG_BITS);
      fprintf(stats_log, "    conf          : %llu bits \n", (uint64_t)ceil(log2((double)(CONF_MAX+1))));
      fprintf(stats_log, "    retired_value : 64 bits \n");
      fprintf(stats_log, "    stride        : 64 bits \n");
      fprintf(stats_log, "    instance ctr  : %llu bits \n", (uint64_t)ceil(log2((double)VPQ_SIZE)));
      fprintf(stats_log, "    ============================\n");
      uint64_t SVPEntryData = SVP_TAG_BITS + (uint64_t)ceil(log2((double)(CONF_MAX+1))) + 64 + 64 + (uint64_t)ceil(log2((double)VPQ_SIZE));
      fprintf(stats_log, "    bits/SVP entry: %llu bits \n", SVPEntryData);

      fprintf(stats_log, "  One VPQ entry: \n");
      fprintf(stats_log, "    PC_tag        : %d bits \n", SVP_TAG_BITS);
      fprintf(stats_log, "    PC_index      : %d bits \n", SVP_INDEX_BITS);
      fprintf(stats_log, "    value         : %d bits \n", 64);
      fprintf(stats_log, "    ============================\n");
      uint64_t VPQEntryData = SVP_TAG_BITS+SVP_INDEX_BITS+64;
      fprintf(stats_log, "    bits/VPQ entry: %llu bits \n", VPQEntryData);

      uint64_t TotalBits = ((uint64_t)(pow(2, SVP_INDEX_BITS)) * SVPEntryData) + (VPQ_SIZE * VPQEntryData);
      fprintf(stats_log, "  Total storage cost (bits) = %llu (%llu SVP entries x %llu bits/SVP entry) + %llu (%d VPQ entries x %llu bits/VPQ entry) = %llu bits\n",
         ((uint64_t)(pow(2, SVP_INDEX_BITS)) * SVPEntryData),(uint64_t)(pow(2, SVP_INDEX_BITS)),SVPEntryData,(VPQ_SIZE * VPQEntryData), VPQ_SIZE,VPQEntryData,TotalBits);
      fprintf(stats_log, "  Total storage cost (bytes) = %.2f B (%.2f KB) \n", (double)((double)TotalBits/8), (double)((double)TotalBits/8192));
}
else if (PERFECT_VALPRED){
    printf("VALUE PREDICTOR = perfect\n\n");
