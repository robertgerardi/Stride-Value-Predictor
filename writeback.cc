  ...
  if (Execution_Lanes[lane_number].wb.valid) {
    ...
    if (PAY.buf[index].checkpoint) {
      ...
      if (PERFECT_BRANCH_PRED) {
        ...
        FetchUnit->mispredict(
            PAY.buf[index].pred_tag,
        ...
        VPUnit.resolveVPQ(PAY.buf[index].VPQIndex, PAY.buf[index].VPQPhase);

        // FIX_ME #15c END

        // Restore the LQ/SQ.
        LSU.restore(PAY.buf[index].LQ_index, PAY.buf[index].LQ_phase,
